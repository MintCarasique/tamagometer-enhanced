...
class AppViewModel(QObject):
    ...
    def __init__(
        ...
        self._transfer_state = "idle"
        self._transfer_status = "Ready"
        self._progress = 0.0
        self._busy = False
        self._pending_key = ""
        self._notice_summary = ""
        self._notice_detail = ""
        self._notice_severity = "info"
        self._poll_ticks = 0
        self._log = []
        self._onboarding_step = 0
        self._onboarding_state = "Have a USB data cable and your Flipper ready."
        self._onboarding_visible = (
            not self._settings.onboarding_complete
            and not self._settings.onboarding_skipped
        )
        self._onboarding_ready = False
        self._setup_connect_pending = False
        self._settings_visible = False
        self._diagnostics_visible = False
        self._timer = QTimer(self)
        self._timer.setInterval(80)
        self._timer.timeout.connect(self.drainEvents)
        if start_timer:
            self._timer.start()
        self.refreshPorts()
        candidate = find_flipper_port(self._ports, self._settings.port)
        if candidate:
            self._selected_port = candidate.device
            self.portsChanged.emit()
            if self._settings.auto_connect:
                self._begin_connect(candidate.device)
    ...
    @Slot(object)
    def _save_favorites(self, values):
        self._save(favorites=tuple(values))

    def _notice(self, severity, summary, detail=""):
        self._notice_severity = severity
        self._notice_summary = summary
        self._notice_detail = detail
        self.noticeChanged.emit()
        self._append_log(
            f"{severity.upper()}: {summary}" + (f" ({detail})" if detail else "")
        )
    ...
    def modes(self):
        return [{"key": m.key, "label": m.selector_label} for m in MODES.values()]
    ...
    def setMode(self, key):
        mode = get_mode(key)
        if mode.key == self._mode.key or self._busy:
            return
        self._mode = mode
    ...
    def toggleTheme(self):
        self._save(theme="light" if self.darkTheme else "dark")
        self.themeChanged.emit()

    @Property(bool, notify=uiChanged)
    def autoConnect(self):
        return self._settings.auto_connect

    @Slot(bool)
    def setAutoConnect(self, value):
        self._save(auto_connect=bool(value))
        self.uiChanged.emit()
    ...
    def setReducedMotion(self, value):
        self._save(reduced_motion=bool(value))
        self.uiChanged.emit()

    @Property(bool, notify=uiChanged)
    def onboardingVisible(self):
        return self._onboarding_visible

    @Property(int, notify=uiChanged)
    def onboardingStep(self):
    ...
    @Property(bool, notify=uiChanged)
    def onboardingReady(self):
        return self._onboarding_ready
    ...
    @Slot()
    def closeOnboarding(self):
        self._onboarding_visible = False
        self.uiChanged.emit()

    @Slot()
    def skipOnboarding(self):
        self._save(onboarding_complete=False, onboarding_skipped=True)
    ...
    def onboardingNext(self):
        if self._onboarding_ready:
            self._save(onboarding_complete=True, onboarding_skipped=False)
            self.closeOnboarding()
            return
        ...
        if not self._selected_port:
            self._onboarding_state = "No verified Flipper found. Check USB, close qFlipper, open the Enhanced app, then retry."
            self.uiChanged.emit()
            return
        self._setup_connect_pending = True
        self._onboarding_state = "Checking Companion compatibility…"
        self.uiChanged.emit()
    ...
    def _finish_setup_connection(self, ok):
        if not self._setup_connect_pending:
            return
        self._setup_connect_pending = False
        if ok:
            self._mark_setup_ready()
            ...
            self._onboarding_state = (
                "Connection failed. Check the cable and Companion, then retry."
            )
            self.uiChanged.emit()
    ...
    def diagnosticsText(self):
        return build_diagnostic_report(
            self._settings, self._connection, self._ports, "\n".join(self._log)
        )

    @Slot()
    def copyDiagnostics(self):
        QGuiApplication.clipboard().setText(self.diagnosticsText)
        self._notice("success", "Diagnostics copied to the clipboard.")

    @Slot(str)
    def exportDiagnostics(self, url):
        path = QUrl(url).toLocalFile() if url.startswith("file:") else url
        try:
            Path(path).write_text(self.diagnosticsText, encoding="utf-8")
        except OSError as error:
            self._notice("error", "Could not export diagnostics.", str(error))
        else:
            self._notice("success", "Diagnostic report exported.")

    @Property("QVariantList", notify=portsChanged)
    def ports(self):
        return [{"device": p.device, "label": p.display_name} for p in self._ports]
    ...
    def selectedPortIndex(self):
        return next(
            (i for i, p in enumerate(self._ports) if p.device == self._selected_port),
            -1,
        )

    @Slot(str)
    def setSelectedPort(self, value):
        self._selected_port = value
        self.portsChanged.emit()

    @Slot()
    def refreshPorts(self):
        self._ports = list(self._port_provider())
        devices = {p.device for p in self._ports}
        if self._selected_port not in devices:
            candidate = find_flipper_port(self._ports, self._settings.port)
            self._selected_port = candidate.device if candidate else ""
        self.portsChanged.emit()
    ...
    def _set_connection_state(self, state, status):
        # Notifications stay with callers, after related state has been updated.
        self._connection_state = state
        self._connection_status = status

    def _set_disconnected(self):
        self._set_connection_state("disconnected", "Flipper disconnected")

    def _begin_connect(self, port):
        if not port or self._connection_state in {"connecting", "connected"}:
            return
        self._set_connection_state("connecting", f"Checking Companion · {port}")
        self.connectionChanged.emit()
        self.transferChanged.emit()

        def worker():
            try:
                self._connection_results.put((True, port, self._connection.open(port)))
            except Exception as error:
                self._connection_results.put((False, port, error))
    ...
    def toggleConnection(self):
        ...
        if not self._selected_port:
            self._notice(
                "error",
                "No verified Flipper port selected.",
                "Connect Flipper, open the Enhanced app, then refresh ports.",
            )
            return
        self._begin_connect(self._selected_port)

    @Property(str, notify=transferChanged)
    def transferState(self):
    ...
    def canStartTransfer(self):
        return (
            self.connected
            and not self._busy
            and (self.legacyMode or self._catalog.selectedItemId >= 0)
        )

    @Property(bool, notify=transferChanged)
    def canCancelTransfer(self):
        return self._busy

    @Property(bool, notify=transferChanged)
    def canRepeatTransfer(self):
        return (
            self.connected
            and not self._busy
            and parse_item_key(self._settings.last_transfer) is not None
        )

    @Property(str, notify=transferChanged)
    def primaryActionHint(self):
        if not self.connected:
            return "Connect a verified Flipper before starting."
        if self._busy:
            return "Transfer active — keep both devices aligned."
        return "Ready. Follow the placement instructions, then start."

    @Slot()
    def startTransfer(self):
        if not self.canStartTransfer:
            return
        item_id = 0 if self.legacyMode else self._catalog.selectedItemId
        name = (
            "Automatic game or gift" if self.legacyMode else self._catalog.selectedName
        )
        self._pending_key = item_key(self._mode.key, item_id)
        self._progress = 0
        self._busy = True
        self._transfer.start(self._mode, item_id, name)
        self.transferChanged.emit()
    ...
    def repeatLastTransfer(self):
        ...
        if not parsed or not self.connected or self._busy:
            return
        mode, item_id = parsed
        self.setMode(mode.key)
        if mode.key != "legacy":
            self._catalog.select_key(item_key(mode.key, item_id))
        self.startTransfer()

    @Property(str, notify=noticeChanged)
    def noticeSummary(self):
        return self._notice_summary

    @Property(str, notify=noticeChanged)
    def noticeDetail(self):
        return self._notice_detail

    @Property(str, notify=noticeChanged)
    def noticeSeverity(self):
    ...
    def _record_success(self):
        if not self._pending_key:
            return
        recent = [k for k in self._settings.recent if k != self._pending_key]
        recent.insert(0, self._pending_key)
        self._save(recent=tuple(recent[:12]), last_transfer=self._pending_key)
        self._catalog.set_recent(self._settings.recent)
        self._pending_key = ""
    ...
    def _handle_connection_result(self, ok, port, payload):
        if ok:
            self._set_connection_state(
                "connected", f"Connected · {port} · Companion {payload.version}"
            )
            self._save(port=port)
            self._notice("success", "Flipper connected and ready.")
        else:
            self._set_connection_state("error", "Flipper connection failed")
            summary = (
        ...
        self._finish_setup_connection(ok)
        self.connectionChanged.emit()
        self.transferChanged.emit()
